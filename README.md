...
Download [Hafþi 0.1.0 beta 24 for Apple Silicon](https://github.com/pdahlbeck/hafthi-mac/releases/tag/v0.1.0-beta.24). Unzip `Hafthi-0.1.0-beta.24-macOS-arm64.zip` and drag `Hafþi.app` to Applications. It requires macOS 13 or later. You do **not** need Xcode or build tools to run the download.
...
| Ghost Tasks | Prefix a command with `g` to run it in a background PTY; open its drawer with Control + G when it needs input. Check status with `g jobs` and read captured output with `g log ID`. |
...
Prefix a command with `g` to run it without mixing output into your main prompt, for example `g make -j4` or `g brew upgrade`. Every new job gets its own PTY in a drawer in the original window. Any command started with `g` can ask for a password, confirmation or other input, even long after it starts. Press **Control + G** to slide the drawer down and answer there; press it again to hide the drawer while the job continues. The shell underneath remains usable. One Ghost Task can run per window at a time; wait for it to finish before starting the next. The completed task stays visible in the drawer until another starts.

Run `g jobs` to check status, `g log ID` to read captured output, `g follow ID` to watch it live (Control + C stops watching), and `g clean` to remove completed logs. Jobs are numbered `ghost1`, `ghost2`, and so on. Quote pipelines or expressions: `g 'make && echo done'`. Tasks run in the directory where you started them. The badge at the upper right blinks between `{ö}` and `{-}` while a task runs; a desktop notification announces completion. Logs are private to your account in `~/Library/Application Support/Hafthi/GhostTasks`. The `g` command is available inside Hafþi.
